...
<body class="bg-light">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input type="text" id="orderSearch" class="form-control form-control-sm"
                            placeholder="Search Order #" style="width:180px;">
                    </div>
                </div>
                <div class="mt-2">
                    <div class="btn-group btn-group-sm" role="group">
                        ...
                        <button type="button" class="btn btn-outline-secondary status-filter-btn" data-status="UNPAID"
                        ...
                        <button type="button" class="btn btn-outline-secondary status-filter-btn" data-status="CLAIMED"
            ...
            <div class="card-body p-0">
                <div class="table-scroll">
                    ...
                    foreach ($orders as $o):
                        ...
                        if ($rawStatus === '' || $rawStatus === 'PENDING' || $rawStatus === 'NEW') {
                            $status = 'UNPAID';
                        } else {
                            $status = $rawStatus;
                        }
                    ...
                    ?>
                            <tr data-order-id="<?= (int)$o['id'] ?>"
                                ...
                                <td><?= $paidOn ? htmlspecialchars($paidOn) : '<span class="text-muted">-</span>' ?>
                                </td>
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                                        ...
                                        <table class="table table-sm align-middle">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end">₱<?= number_format((float)$p['price'], 2) ?>
                                                    </td>
                                                    <td class="text-end">
                                ...
                                <div class="card-body py-2">
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group input-group-sm" style="max-width:220px;">
                                            ...
                                            <input type="text" id="cashInput" class="form-control" placeholder="0"
                                                autocomplete="off">
                                        </div>
                                        <div class="form-text">
                ...
                <div class="modal-footer py-2">
                    ...
                    <button type="button" class="btn btn-success btn-sm" onclick="proceedPayment()">
                        Proceed
                    </button>
                </div>
            </div>
    ...
    <div id="printReceiptArea" style="display:none;">
        <div style="width:350px; padding:16px; font-family:monospace;">
            <h2 style="text-align:center;">Your Shop Name</h2>
            <hr>
            <div>
                <strong>Order #<span id="printReceiptOrderId"></span></strong><br>
                Date: <span id="printReceiptDate"></span><br>
                Teller: <?= htmlspecialchars($_SESSION['employee_name']) ?><br>
            </div>
            <hr>
            <table style="width:100%; font-size:15px; margin-bottom:8px;">
                <thead>
                    <tr>
            ...
            <div style="text-align:right; font-size:17px;">
                <strong>Total: ₱<span id="printReceiptTotal"></span></strong>
            </div>
            <div style="text-align:right;">
                Cash: ₱<span id="printReceiptCash"></span><br>
                Change: ₱<span id="printReceiptChange"></span>
            </div>
            <hr>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="confirmPaymentLabel">Review Order Payment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="proceedSummaryContent"></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">
                        Back
                    </button>
                    <button type="button" class="btn btn-dark btn-sm" onclick="printReceipt()">
                        Print Receipt
                    </button>
                    <button type="button" class="btn btn-success btn-sm" onclick="confirmPaymentFinal()">
                        Confirm Payment
                    </button>
                </div>
            </div>
    ...
    function applyFilters() {
        ...
        document.querySelectorAll('#ordersTbody tr[data-order-id]').forEach(tr => {
            ...
            const status = tr.dataset.status ? tr.dataset.status.toUpperCase() : '';
            let visible = true;

            if (q && !disp.includes(q)) {
                visible = false;
            }
            // Filter must use uppercase to match the normalized data
            if (currentStatusFilter !== 'ALL' && status !== currentStatusFilter.toUpperCase()) {
                visible = false;
            }
    ...
    // Called when user first clicks Proceed
    function proceedPayment() {
        ...
        let summaryHTML = `
    <div style="width:280px; margin:auto; font-family: monospace; font-size: 14px; line-height: 1.3;">
        
        <!-- Header -->
        <div style="text-align:center; margin-bottom:5px;">
            <strong style="font-size:18px;">Your Shop Name</strong><br>
            <span>123 Street, Your City</span><br>
            <span>Tel: 0912-345-6789</span>
        </div>
        ...
        <div style="margin-bottom:8px;">
            <strong>Order #${currentOrderSeq}</strong><br>
    ...
    `;
        ...
        document.getElementById('proceedSummaryContent').innerHTML = summaryHTML;
        confirmPaymentModal.show();
    }


    // Called when user CONFIRMS inside summary modal
    function confirmPaymentFinal() {
        // Everything validated in proceedPayment already
    ...
    function printReceipt() {
        if (!currentOrderId || !orderItems.length) {
        ...
        // Fill printable area
        document.getElementById('printReceiptOrderId').textContent = currentOrderSeq;
        document.getElementById('printReceiptDate').textContent =
        ...
        document.getElementById('printReceiptCash').textContent = cash.toFixed(2);
        document.getElementById('printReceiptChange').textContent = change > 0 ? change.toFixed(2) : '0.00';

        // Print only the receipt div
        const printContents = document.getElementById('printReceiptArea').innerHTML;
        const tempWin = window.open('', '', 'width=400,height=600');
